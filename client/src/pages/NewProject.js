...
const blankQ = () => ({ text: '', assignedToId: '', maxLimit: '', limitUnit: 'words' });
...
export default function NewProject() {
  ...
  const addFromBulk = () => {
    ...
    setQuestions(qs => [...qs.filter(q => q.text.trim()), ...lines.map(text => ({ ...blankQ(), text }))]);
    setBulk('');
    setShowBulk(false);
  };
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await api.post('/api/projects', {
        ...
        questions: qs.map(q => ({ text: q.text, assignedToId: q.assignedToId || null, maxLimit: q.maxLimit || null, limitUnit: q.limitUnit })),
      });
          ...
          <div className="card-body">
            {showBulk && (
              <div className="callout mb-3">
                <Field label="Paste questions, one per line" hint="Numbering and bullets are stripped automatically.">
                  <Textarea value={bulk} onChange={e => setBulk(e.target.value)} rows={6} placeholder={'1. Describe your organization.\n2. What is the total project budget?'} />
                </Field>
            ...
            <Button variant="secondary" size="sm" className="mt-2" onClick={() => setQuestions(qs => [...qs, blankQ()])}>+ Add question</Button>
          </div>
        </Card>
