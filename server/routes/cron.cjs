    ...
    for (const c of endedDue) {
      ...
      for (const a of admins) {
        await sendEmail({
          ...
          html: layout('Your trial has ended', `<p>Hi ${a.name || a.username}, the trial for <strong>${c.name}</strong> is over and the workspace is now on the Free plan.</p><p>Everything you wrote is still there. You can keep writing and downloading your existing projects. To bring everything else back, pick a plan any time.</p>${button(appUrl('/app/settings#plan'), 'See plans')}`),
