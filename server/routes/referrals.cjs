...
router.get('/mine', auth, async (req, res) => {
  try {
    let c = await prisma.company.findUnique({ where: { id: req.user.companyId }, select: { id: true, name: true, plan: true, kind: true, trialEndsAt: true, compedUntil: true, isStaff: true, referralCode: true } });
    const ok = eligible(c);
    if (ok && !c.referralCode) {
      ...
      c = await prisma.company.update({ where: { id: c.id }, data: { referralCode: code }, select: { id: true, name: true, plan: true, kind: true, trialEndsAt: true, compedUntil: true, isStaff: true, referralCode: true } });
    }
...
router.get('/check/:code', async (req, res) => {
  try {
    const c = await prisma.company.findUnique({ where: { referralCode: String(req.params.code).toUpperCase() }, select: { name: true, plan: true, kind: true, trialEndsAt: true, compedUntil: true, isStaff: true } });
...
async function recordReferral(referredCompanyId, code) {
  if (!code) return;
  const referrer = await prisma.company.findUnique({ where: { referralCode: String(code).toUpperCase() }, select: { id: true, plan: true, kind: true, trialEndsAt: true, compedUntil: true, isStaff: true } });
